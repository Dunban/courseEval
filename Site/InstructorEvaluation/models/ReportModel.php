...
class ReportModel
{
	...
	function getAllEvaluations($userID)
	{
		...
		$sql = 
			"SELECT a.evaluationID, name, quarter, year, section, count(c.evaluationID) as taken, b.expirationDate, b.code".
	...
	public function getEvaluationDetails($evaluationID)
	{
		...
		//$evalInfo = $this->getEvalInfo($evaluationID);
	...
	public function getEvalInfo($evaluationID){
		$db = $this->router->get('DB');
		$sql = 
			"SELECT name, quarter, year, section, count(c.evaluationID) as taken, b.expirationDate, b.code".
			" FROM evaluationInfo a".
			" LEFT JOIN evaluationCodes b on a.evaluationID = b.evaluationID".
			" LEFT JOIN usedEvaluationCodes c on a.evaluationID = c.evaluationID".
	...
	private function formatEvaluationList($list)
	{
		foreach ($list as &$item) {
			...
			if(!empty($item['expirationDate']))
			{
				...
				$item['expirationDate'] = $temp->format('m-d-Y H:i');
			}
		}
		return $list;
	}
